...
namespace FinalProject.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult ResetPassword(string id)
        {
            if (id == null)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest,"Could not find user");
            }
            var removePassword = UserManager.RemovePassword(id);
            if (removePassword.Succeeded)
            {
                //Removed Password Success
                var addPassword = UserManager.AddPassword(id,"a2zpassword");
                if (addPassword.Succeeded)
                {
                    return RedirectToAction("Index", "Users", new  { message = "Password Reset done successfully" });
                }
            }
